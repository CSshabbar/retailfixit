# RetailFixIt — Engineering Reasoning & Operational Policies (Part 3)
...
**Distributed Traces (for latency troubleshooting):**

Each API request should carry a correlation ID (generated by the mobile client or assigned by the backend). This ID links:
1. Mobile → API request log (client-side timing)
2. API → Cosmos DB query log (database timing)
3. API → SignalR broadcast log (notification timing)
4. API → Blob Storage operation log (upload timing)

Currently, Pino-HTTP provides per-request logging with automatic request IDs. For production, integrating with Azure Application Insights would add end-to-end distributed tracing with dependency tracking, custom events, and alerting dashboards.
